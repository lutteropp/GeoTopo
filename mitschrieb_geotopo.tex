...
Eine \underline{Homotopie} $H \colon f \simeq g$ zwischen zwei (stetigen) Abbildungen $f,g \colon X \rightarrow Y$ ist eine (stetige) Abbildung $$H \colon X \times I \footnote{$I = [0,1] \subset \R$} \rightarrow Y, (x,t) \mapsto H(x,t)$$ mit $H(x,0) = f(x) \text{ und } H(x,1) = g(x) \quad \forall x \in X$.
\end{Kasten}
...
\newpage
\section{Grundlagen der allgemeinen Topologie} % TODO: Römisch 1
\begin{bsp}{Beispiele topologischer Räume}
	\begin{enumerate}[(1)]
...
% diese Woche noch nicht relevant, hört auf mit diffbaren Mfkten
...
\begin{bsp}{}
	...
	$$\Rightarrow X/A \cong \includegraphics[scale=0.2]{images/1_nach_1durch3.jpg} \cong \includegraphics[scale=0.2]{images/1_nach_1durch3_2.jpg} \cong \includegraphics[scale=0.2]{images/P.jpg} \cong \includegraphics[scale=0.2]{images/Q.jpg}$$
		...
		\item $X = \overline{B^n}$ abgeschlossener Einheitsball in $\R^n$ mit Rand $S^{n-1}$ und Inneren $B^n$. $A:= S^{n-1}= \partial \overline{B^n}, X = \overline{B^n}$ \newline
...
\begin{Kasten}{$n$-dimensionaler reell-projektiver Raum}
	Der $n$-dimensionale reell-projektive Raum\footnote{Anschaulich (projektive Geometrie): 
...
\begin{remark}{}
	$C(X,Y)$ ist, versehen mit der sogenannten \underline{kompakt-offenen} \underline{Topologie}, selbst ein topologischer Raum und jede Homotopie $H \colon f \simeq g, \quad f,g \in C(X,Y)$ ist bezüglich dieser Topologie auf $C(X,Y)$ dann tatsächlich als \underline{stetige} Abbildung (also Weg) $I \rightarrow C(X,Y)$ interpretierbar.
\newline
...
\begin{Kasten}{Produkt von Wegen}
	...
	$$(u \cdot v) (t) := \begin{cases} u(2t) & 0 \leq t \leq \frac{1}{2} \\ v(2t-1) & \frac{1}{2} \leq t \leq 1 \end{cases}$$
...
\begin{Kasten}{Konstanter Weg, Inverser Weg, Geschlossener Weg}
	\begin{itemize}
		...
		\item Für einen Weg $u \colon I \rightarrow X$ sei $u^{-1} \colon I \rightarrow X, t \mapsto u(1-t)$, der zu $u$ \underline{inverse} (oder: \underline{umgekehrt durchlaufene}) \underline{Weg}. \includegraphics[scale=0.4]{images/inverser_Weg.jpg}
...
\begin{Kasten}{nullhomotop, einfach zusammenhängend}
	...
	\item $X$ heißt \underline{einfach zusammenhängend} \newline
	$:\Leftrightarrow \text{ jeder geschlossene Weg $u$ in $X$ ist nullhomotop (zu $c_{u(0)}$).}$
	\end{itemize}
\end{Kasten}
...
\begin{remark}
	...
	\newline
$[w^{-1}][u \cdot v][w] = [w^{-1} u w] [w^{-1} v w]$
\newline
$\overset{\text{abelsch}}{=} [u v] [w w^{-1}] = [u v]$
...
\begin{remark}{}
	...
	\newline
	Denn ist $u \colon I \rightarrow X$ geschlossener Weg in $x_0 \in X$, so existiert eine Quotientenabbildung $\tilde{u}\colon I/_{\{0,1\}} \rightarrow X$ und $I/_{\{0,1\}} \simeq S^1!$
	(TODO: Bild 4)
\end{remark}
...
\begin{Kasten}{schleifenhomotop}
	...
	$\gamma(1) = x_0 = \gamma^\prime(1)$ die ganze Zeit festhält. (TODO: Bild 3)
\end{Kasten}
...
\begin{bsp}{}
	...
	\newline
	Denn zwischen jeder Schleife in $0 \in \R^n$ und $c_0$ gibt es eine Homotopie. (TODO: Bild 6)
\end{bsp}
...
\begin{bsp}{}
	Für jede \underline{konvexe} oder auch \underline{sternförmige} Teilmenge $K \subset \R^n$ und alle $x_0 \in K$ ist $\pi_1(K,x_0)$ trivial. (TODO: Bild 7)
\end{bsp}
...
\begin{bsp}{}
	...
	(TODO: Bild 8)
	\newline
	\underline{Achtung:} $\forall n \in \N$ existieren stetige Surjektionen $I \rightarrow S^n$ (TODO: Bild 9)
	\newline
...
\begin{enumerate}
	...
	\newline
	\underline{Denn:} Es sei $p \in S^n \backslash u(I)$ und $\pi \colon S^n \backslash \{p\} \rightarrow \R^n$ die stereographische Projektion (von $p$ aus). Dann ist $\pi \circ u$ geschlossener Weg in $\R^n$ und nullhomotop in $\R^n$. (TODO: Bild 10)
	\newline
	...
	\item Jeder stetige Weg auf $S^n$ ist homotop zu einem nicht surjektiven Weg auf $S^n$!
\end{enumerate}

\paragraph{Erinnerung(Lebesquelemma)}
Ist $f \colon Z \rightarrow Y$, $Z$ kompakter metrischer Raum, $Y$ topologischer Raum und $\Gamma$ eine offene Überdeckung von $Y$, so existiert $\delta > 0$ mit: \newline  $\forall A \subset Z \text{ mit } diam(A) < \delta$ ist $f(A)$ in einem Element von $\Gamma$ (ganz) enthalten. (TODO: Bild 11)
...
\begin{lemma}{}
	...
	\end{lemma}
	\begin{proof}
	\underline{Denn:} Sei $x \in S^n$ beliebig und überdecke $S^n$ durch $U:= S^n \backslash \{x\}$ und $V:= S^n \backslash \{-x\}$. Korollar zu Lebesque-Lemma $\Rightarrow$ $\exists a_0, \ldots, a_N \in I, 0 = a_1 < \ldots < a_N = 1 \colon \forall i$ liegt $s([a_i, a_{i+1}])$ ganz in $U$ oder $V$.
	\newline
	$U,V \cong \R^n$ (hier sind alle Wege homotop) $\Rightarrow \forall i \colon s \big |_{[a_i, a_{i+1}]} \sim$ Weg, der Teil eines Großkreises auf $S^n$ ist. (TODO: Bild 13) Letzterer füllt nicht $S^n$.
\end{proof}
\end{document}
