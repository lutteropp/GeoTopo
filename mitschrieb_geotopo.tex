...
\begin{definition}[Stetigkeit] %spacings stimmen nicht ganz (TODO)
...
Entfernt man aus $S^1$ einen Punkt oder auch ein abgeschlossenes Kreissegment (TODO: Bild 1), so erhält man eine offene Teilmenge $U$ von $S^1$, deren Urbild unter $\pi$ aus disjunkten offenen Intervallen besteht, und jedes dieser Intervalle wird unter $\pi$ homöomorph auf $U$ abgebildet.

\paragraph{Veranschaulichung}
Indentifiziere $\R$ zunächst mit einer "Spirale" im $\R^3$ via $t \mapsto (\cos 2 \pi t, \sin 2 \pi t, t);$ (TODO: Bild 2)
...
\begin{definition}[Überlagerung]
	...
	$$\pi \big |_{\tilde{U}_\lambda \colon \tilde{U}_\lambda \rightarrow U(X)}$$ ein Homöomorphismus ist. (TODO: Bild 3)
