...
\begin{remark}
	Ist $f \colon X \rightarrow Y$ eine stetige Abbildung zwischen topologischen Räumen, so liefert jeder Weg in $X$ einen Weg in $Y$, (TODO: Bild 1) und für aneinanderfügbare Wege $u,v \colon I \rightarrow X$ gilt offenbar $f \circ (u \cdot v) = (f \circ u) \cdot (f \circ v)$.
	\newline
...
\begin{proof}
	$p_1 \colon X \times Y \rightarrow X$ und $p_2 \colon X \times Y \rightarrow Y$ seinen die Projektionen von $X \times Y$ auf den erstem bzw. zweiten Faktor. (TODO: Bild 3) \newline Ist $Z$ irgendein anderer topologischer Raum, so entspricht jeder stetigen Abbildung $f \colon Z \rightarrow X \times Y$ bijektiv ein Paar $(f_1, f_2)$ stetiger Abbildungen $f_1 \colon Z \rightarrow X, f_2 \colon Z \rightarrow Y$ mit $f_i = p_i \circ f, \quad i=1,2$
	\newline
...
\begin{theorem}
	...
	\begin{enumerate}
		...
		\item Je zwei Wege $u_1, u_2$ in $X$ mit gleichen Anfangs- bzw. Endpunkten sind homotop. (TODO: Bild 4)
	\end{enumerate}
\end{theorem}
