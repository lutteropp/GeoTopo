...
\begin{remark}
Die Äquivalenzrelation "Homotopie von Abbildungen" liefert also eine Partition von $C(X,Y)$ in Äquivalenzklassen. Diese heißen Homotopieklassen und die \underline{Menge aller Homotopieklassen} \underline{stetiger Abbildungen von $X$ nach $Y$} wird mit $[X,Y]$ bezeichnet.
(TODO: BILD)
\end{remark}
...
\begin{example}[Beispiele topologischer Räume]
	\begin{itemize}
		...
		\item (6) $X:= \R, \tilde{\OO} := \{O \mid O = \R \backslash E, E \subset \R \text{ endlich}\} \cup \{\emptyset\}$ ist auch eine Topologie auf $\R$, die so genannte $T_1$-Topologie.
	\end{itemize}
\end{example}
...
\begin{example}
	In einem diskreten topologischen Raum sind \underline{alle Teilmengen} abgeschlossen, in $\R_{T_1}$\footnote{$\R$ mit $T_1$-Topologie} alle endlichen Teilmengen und $X, \emptyset$.
\end{example}
...
\begin{example}
	...
	\newline
	$\bullet$ Die Standard-Topologie auf $\R$ ist feiner als die $T_1$-Topologie.
\end{example}
...
\begin{Kasten}{Rand}
	Der \underline{Rand} $\delta A$ von $A$ ist $\delta A := \bar{A} \backslash \mathring A$, d.h. Rand $A$ = \{ Randpunkte von A \}.
\end{Kasten}
...
\begin{example}{Stereoraphische Projektion}
	(TODO:BILD)
	\newline
