...
\begin{example}
	\begin{itemize}
		...
		\item $[0,1) \rightarrow S^1$ ist \underline{keine} Einbettung! (TODO: Bild)
...
\begin{Kasten}{Äquivalenz von Einbettungen}
	Zwei Einbettungen $f,g \colon X \rightarrow Y$ heißen \underline{äquivalent} $:\Leftrightarrow \exists \text{ Homöomorphismen } h_X \colon X \rightarrow X, h_Y \colon Y \rightarrow Y \text{ mit } g \circ h_X = h_Y \circ f$, d.h. dass das Diagramm (TODO:Bild) kommutiert.
\end{Kasten}
...
\begin{example}
	Die Knoten mit Bildern (TODO:Bild) und (TODO:Bild) sind äquivalent, die mit (TODO:Bild) und (TODO:Bild) nicht!
\end{example}
...
\begin{Kasten}{Partition}
	Eine \underline{Partition} einer Menge ist eine Überdeckung dieser Menge durch paarweise disjunkte Teilmengen.
	\newline
	(TODO:Bild)
\end{Kasten}
...
\begin{remark}
	...
	$\Leftrightarrow$ es existiert keine Partition von $X$ in zwei nichtleere abgecshlossene Teilmengen
	\newline
...
\begin{proof}
	...
	\newline
	Sei $f^{-1}(U) = \emptyset \Rightarrow U = f(\emptyset) = \emptyset \Rightarrow V = f(X)$ ($f$ surjektiv auf $f(X)$)
	\newline
...
\begin{example}
	...
	\newline
	\underline{Denn:} $\R^n \cong \mathring{D}^n$\footnote{Einheitskugel} und nimmt man aus $\mathring{D}^n$ einen Punkt $p$ heraus, so bleibt für $n>1$ $\mathring{D}^n \backslash \{p\}$ zusammenhängend, $\mathring{D}^1 = (-1,1) \cong \R$ aber nicht!
	\newline
	(TODO:Bild)
	\newline
