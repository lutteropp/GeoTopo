...
\begin{center}
\includegraphics[width=0.3\textwidth]{images/stetigeAbb.png}
\end{center}
\end{definition}
...
\newline
\begin{center}
\includegraphics[width=0.4\textwidth]{images/Homotopie.png}
\end{center}
\end{definition}
...
\begin{figure}[h]
\centering
\subfloat{\includegraphics[width=0.5\textwidth]{images/nicht_homotop_1.png}}\qquad
\subfloat{\includegraphics[width=0.4\textwidth]{images/nicht_homotop_2.png}}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.3\textwidth]{images/Homotopie_param.png}
...
\begin{center}
\includegraphics[width=0.3\textwidth]{images/Nullhomotopie.jpg}
\end{center}
\end{definition}
...
\begin{definition}{Basis der Topologie}
...
\begin{definition}{$\epsilon$-Ball, Sphäre}
	...
	\begin{itemize}
		\item $B_\epsilon(p):=\{x \in X \mid d(p,x) < \epsilon \}$ der \underline{offene $\epsilon$-Ball um $p$}
		\item $D_\epsilon(p):=\{x \in X \mid d(p,x) \leq \epsilon \}$ der \underline{abgeschlossene $\epsilon$-Ball um $p$}
		\item $S_\epsilon(p):=\{x \in X \mid d(p,x) = \epsilon \}$ die \underline{ $\epsilon$-Sphäre} um $p$ (oder \underline{Sphäre vom Radius $\epsilon$} um $p$)
	\end{itemize}
\end{definition}
...
\begin{definition}{Beschränktheit, Durchmesser}
	...
	\newline
	\begin{center}
	\includegraphics[width=0.3\textwidth]{images/Durchmesser.jpg}
	\end{center}
\end{definition}
...
\begin{remark}
	...
	\newline
	$\Leftrightarrow \forall p \in A \quad \exists \text{ ein offener Ball } B_\epsilon(p) \text{ um } p \text{ mit } B_\epsilon(p) \subset A$
	\newline
	\begin{center}
	\includegraphics[width=0.3\textwidth]{images/metrisch_offen.jpg}
	\end{center}
\end{remark}
...
\begin{remark}
	...
	Ist $A \subset X$ \underline{offen} \underline{\underline{in $X$}}, so ist auch jede in $A$ offene Menge offen in $X$, und abgeschlossene\footnote{in $A$} Teilmengen einer \textbf{in $X$ abgeschlossenen Menge $A$} sind auch abgeschlossen in $X$.
		
		\begin{center}
	\includegraphics[width=0.3\textwidth]{images/offen_abgeschlossen.jpg}
	\end{center}
	Aber abgeschlossene Mengen $B$ in $A \subset X$ sind für \textbf{beliebiges $A$} im Allgemeinen nicht abgeschlossen in $X$.
\end{remark}
...
\begin{definition}{Innerer Punkt, äußerer Punkt, Randpunkt}
	...
	\begin{enumerate}[(1)]
		...
		\item \underline{äußerer Punkt} von $A$, falls eine zu $A$ disjunkte Umgebung $V$ um $p$ in $X$ existiert.
		\item \underline{Randpunkt} von $A$, falls jede Umgebung von $p$ nichtleeren Durchschnitt mit $A$ und $X \backslash A$ hat.
	\end{enumerate}
		\begin{center}
	\includegraphics[width=0.3\textwidth]{images/innen_aussen.jpg}
	\end{center}
\end{definition}
...
\begin{center}
\includegraphics[width=0.4\textwidth]{images/stetig_in_x.jpg}
\end{center}
\end{definition}
...
\begin{bsp}{}
	\begin{itemize}
		...
		\item \underline{Der Satz über die Umkehrabbildung/ Impliziter Funktionensatz} aus der Analysis zeigt: 
			\newline
			Ist $f \colon \R^n \rightarrow \R^n$ stetig differenzierbar und in $p \in \R^n$ die Jacobi-Matrix $Df(p)$ invertierbar, so existiert eine Umgebung $U$ von $p$, auf der $f \big |_U$ eine Einbettung ist.
	\end{itemize}
\end{bsp}
...
\begin{definition}{Äquivalenz von Einbettungen}
	...
	\begin{center}
		\includegraphics[scale=0.3]{images/homotopieaequivalenz.jpg} 
	\end{center}
		kommutiert.
\end{definition}
...
\begin{bsp}{}
	Die Knoten mit Bildern \includegraphics[scale=0.3]{images/Knoten_Kreis.jpg} und \includegraphics[scale=0.3]{images/Knoten_aequivalent_zu_Kreis.jpg} sind äquivalent, die mit Bildern \includegraphics[scale=0.3]{images/Knoten_Kreis.jpg} und \includegraphics[scale=0.3]{images/Knoten_nicht_aequivalent_zu_Kreis.jpg} nicht!
\end{bsp}
...
\begin{center}
	\includegraphics[scale=0.3]{images/Ueberdeckung.jpg}
\end{center}	
...
\begin{definition}{Partition}
	...
	\begin{center}
		\includegraphics[scale=0.3]{images/Partition.jpg}
	\end{center}
\end{definition}
...
\begin{definition}{Weg, Anfangspunkt, Endpunkt}
	...
	\begin{center}
		\includegraphics[scale=0.3]{images/Weg.jpg}
	\end{center}
\end{definition}
...
\begin{bsp}{}
	...
	\newline
	\scriptsize{Bild: http://de.wikipedia.org/w/index.php?title=Datei:Sinuseinsdurchx.jpg\&filetimestamp=20080624085708}
\end{bsp}
...
\begin{center}
	\includegraphics[scale=0.3]{images/lokal_endlich.jpg}
\end{center} 
\end{definition}
...
\begin{definition}{Mannigfaltigkeit, Karte}
	...
	\end{enumerate}
	\begin{center}
	\includegraphics[scale=0.3]{images/Karte.jpg} 
	\end{center} 
\end{definition}
...
\begin{definition}{$C^k$-Atlas, Kartenwechsel}
	...
	\begin{center}
	\includegraphics[scale=0.3]{images/Kartenwechsel.jpg}
	\end{center} 
\end{definition}
...
\begin{definition}{$C^k$-Mannigfaltigkeit, glatt}
	...
	\newline
\end{definition}
\includegraphics[scale=0.3]{images/Verkleben.jpg}
\includegraphics[scale=0.3]{images/Verkleben_S2.jpg}
...
\begin{theorem}[Äquivalente Beschreibungen einer Untermannigfaltigkeit von $\R^{n+l}$]
	...
	\begin{enumerate}[(a)]
		\item $\forall x_0 \in M \quad \exists \text{ Umgebung } U = U(x_0) \subset_{offen} \R^{n+l}$ und $$f \in C^\infty(U, \R^l) := \{g \colon U \rightarrow \R^l \mid g \text{ ist } C^\infty\}\text{ mit Rang }Df(x) = l \quad \forall x \in U$$ \footnote{$Df$ ist die Jacobi-Matrix von $f$} dergestalt, dass $U \cap M = f^{-1}(0) = \{x \in U \mid f(x) = 0\}$ 
	\begin{center}
\includegraphics[scale=0.5]{images/Satz_UnterMf.jpg}
	\end{center}
		\item $\forall x_0 \in M \quad \exists U = U(x) \subset_{offen} \R^{n+l}$ und $\varphi \colon U \rightarrow \R^{n+l}$ mit folgenden Eigenschaften:
		...
		\item $\forall x_0 \in M \quad \exists U = U(x_0) \subset_{offen} \R^{n+l}, W \subset \R^n$ offen und $\psi \in C^\infty(W,U)$ mit 
			\begin{itemize}
	...
	\begin{enumerate}[(a)]
		...
		\item $\forall x_0 \in M \quad \exists U = U(x) \subseteq_{\text{offen}} \R^{n+l} \text{ und } \varphi \colon U \rightarrow \R^{n+l}$ mit folgenden Eigenschaften:
			\begin{itemize}
				...
				\item $\varphi(U \cap M) = \varphi(U) \cap (\R^n \times \{0\})$ $= \{(y_1, \ldots, y_{n+l}) \in \varphi(U) \mid y_{n+1} = \ldots = y_{n+l} = 0\}$
			\end{itemize}
...
\newpage
\paragraph{Erinnerung} Jede Partition \includegraphics[scale=0.1]{images/Partition.jpg} $S$ einer Menge $X$ bestimmt eine Äquivalenzrelation auf $X$ (und umgekehrt).
\newline
...
\begin{bsp}{}
	...
	Ist $\tan \alpha$ rational, so schließen sich die ins Einheitsquadrat "zurückgeholten" Kurvensegmente zu einer geschlossenen Kurve $K_\alpha$ auf dem Torus $T^2$, doch für $\tan \alpha \in \R \backslash \Q$ füllt die entsprechende Kurve $K_\alpha$ den $T^2$ \underline{dicht} aus. $\leadsto$
	$$\tan \alpha \in \Q \Rightarrow K_\alpha \cong S^1$$
	$$\tan \alpha \in \R \backslash \Q \Rightarrow K_\alpha \cong \R \ldots !$$	
	
	\includegraphics[scale=0.5]{images/Linien_Torus_rational.jpg}
	\newline
	$\tan \alpha \in \R \backslash \Q$: \underline{\underline{ÜBEL!}}\newline
	\includegraphics[scale=0.5]{images/Linien_Torus_irrational.jpg}
	\newline
	$t \mapsto e^{2 \pi i t}$
	\newline
...
\begin{bsp}{Die Kleinsche Flasche/ der Kleinsche Schlauch}
	...
	\includegraphics[scale=0.3]{images/Quadrat_Kleinsche_Flasche.jpg} ist Quotient des Zylinders bzw. Möbiusbandes.
	\newline	
	\includegraphics[scale=0.3]{images/Zylinder_nach_Kleinsche_Flasche.jpg}
\end{bsp}

\begin{bsp}{Die projektive Ebene}
	\begin{center}
		\includegraphics[scale=0.3]{images/RP2.jpg}
	\end{center}
...
\begin{center}
\includegraphics[scale=0.3]{images/Homotopie_in_Y.jpg}
\end{center}
...
\begin{definition}{homotop bezüglich der Endpunkte}
	...
	\end{enumerate}
	\begin{center}
		\includegraphics[scale=0.3]{images/Homotop_Endpunkte.jpg}		
	\end{center}
\end{definition}
...
\begin{lemma}{}
	Für Wege $u,v,w \colon I \rightarrow X$ \newline mit $u(0)=x, u(1)=y=v(0), \quad v(1)=z= w(0)$ 
	\begin{center} 
...
\begin{definition}{lokal wegzusammenhängend}
	$Z$ \underline{lokal wegzusammenhängend} $:\Leftrightarrow$ in jeder Umgebung jedes Punktes in $Z$ ist immer eine wegzusammenhängende Umgebung enthalten. \begin{center}
	 	\includegraphics[scale=0.5]{images/2011_12_20_Bild6.jpg}
	 \end{center}
\end{definition}
...
\begin{definition}{Decktransformation}
	Eine \underline{Decktransformation}/\underline{Deckbewegung} einer Überlagerung $\pi \colon \widetilde{X} \rightarrow X$ ist ein Homöomorphismus $f \colon \widetilde{X} \rightarrow \widetilde{X}$ mit $\boxed{\pi = \pi \circ f}$ \begin{center}
	 	\includegraphics[scale=0.5]{images/2011_12_22_Bild4.png}
	 \end{center}
\end{definition}
...
\begin{example}[Der Hawaiianische Ohrring]
	\begin{center}
	 	\includegraphics[scale=0.5]{images/2011_12_22_Bild9.png}
	 \end{center}
...
\section{Topologie von Flächen}

\begin{definition}{Fläche, geschlossen}
	Eine \underline{Fläche} $F$ ist eine zweidimensionale glatte Mannigfaltigkeit.
	\newline
	Ist $F$ kompakt, so heißt $F$ auch \underline{geschlossen}.
\end{definition}

\begin{definition}{orientierbar}
	Enthält eine Fläche einen Unterraum, der homöomorph zu einem Möbiusband ist, so heißt die Fläche \underline{nicht-orientierbar}, ansonsten \underline{orientierbar}.
\end{definition}

\begin{example}
	$S^2, T^2$, \begin{center}
	 	\includegraphics[scale=0.5]{images/2012_01_10_Bild03.png}
	 \end{center} \begin{center}
	 	\includegraphics[scale=0.5]{images/2012_01_10_Bild04.png}
	 \end{center} sind orientierbare geschlossene Flächen.
\end{example}
...
\begin{definition}{Zusammenhängende Summe}
	Es seien $F$ und $F^\prime$ geschlossene Flächen. Schneidet man aus jeder Fläche eine abgeschlossene Scheibe 	 \begin{center}
	 	\includegraphics[scale=0.5]{images/2012_01_10_Bild14.png}
	 \end{center} heraus und identifiziert diese durch einen Homöomorphismus 	 \begin{center}
	 	\includegraphics[scale=0.5]{images/2012_01_10_Bild15.png}
	 \end{center} der Ränder, so heißt der durch diese Identifikation entstehende Raum die \underline{zusammenhängende Summe} $F \# F^\prime$ von $F$ und $F^\prime$. 	 \begin{center}
	 	\includegraphics[scale=0.5]{images/2012_01_10_Bild16.png}
	 \end{center}
\end{definition}
...
\end{corollary}
	...
	\begin{definition}{kombinatorische Unterteilung, Ecken, Kanten, Seiten}
		Eine kombinatorische \underline{Unterteilung} von $F$ ist eine Zerlegung von $F$ in
		\begin{enumerate}
			\item \underline{Ecken} (endlich viele Punkte)
			\item \underline{Kanten} (endlich viele disjunkte Teilmengen von $F$, die alle zum offenen Intervall $(0,1)$ homöomorph sind)
				\begin{center}
					\includegraphics[scale=0.5]{images/2012_01_12_Bild7.png}
			\end{center}
			\item \underline{Seiten} (endlich viele disjunkte offene Teilmengen von $F$, die alle zur offenen Scheibe $\{||x|| < 1\} \subset \R^2)$ homöomorph sind), so dass gilt:
			\newline
			\begin{enumerate}
				\item Die Seiten sind Zusammenhangskomponenten von $F \backslash \{\text{Ecken und Kanten}\}$
				\item keine Kante enthält eine Ecke
				\item jede Kante beginnt und endet in einer Ecke, d.h. ist $k$ Kante, so gibt es Ecken $e_0$ und $e_1$ und eine stetige Abbildung $f \colon [0,1] \rightarrow k \cup \{e_0, e_1\} \subset F$, die eingeschränkt auf $(0,1)$ ein Homöomorphismus ist und $f(0)=e_0, f(1)=e_1$ erfüllt.
			\end{enumerate}
		\end{enumerate}
	\end{definition}
	
	\begin{definition}{kombinatorische Euler-Charakteristik}
		Die (\underline{kombinatorische}) \underline{Euler-Charakteristik} einer geschlossenen Fläche $F$ mit Unterteilung ist
		$$\chi (F) := \# \text{ Ecken } - \# \text{ Kanten } + \#\text{ Seiten}$$
	\end{definition}
...
\begin{example}
	\begin{enumerate}
		\item $r(u,v) = (a+b \cdot \cos u)(\cos v \cdot e_1 ü \sin v e_2) + b \sin u e_3$ (Torus) ($a,b > 0$)
		\item $r(u,v) = a \cdot \sin u \sin v e_1 + a \cos u \sin v e_2 + a \cos v e_3$ (Sphäre)
		\item $r(u,v) = a u \cos v e_1 + a u \sin v e_2 + v e_3$ (Helikoid)
	\end{enumerate}
\end{example}

%hier beginnt der 19.01.2012

\paragraph{}

\begin{center}
	\includegraphics[scale=0.5]{images/2012_01_19_Bild1.png}
\end{center}

Ein \underline{Kartenwechsel} entspricht nun einer \underline{Umparametrisierung} $$r \circ f \colon V^\prime \rightarrow \R^3, f \colon V^\prime \rightarrow V \subset \R^2 \text{ Diffeomorphismus}$$
Gilt $f(x,y) = (u(x,y), v(x,y))$, so ist
$$(r \circ f)_x = r_u u_x + r_v u_x$$
$$(r \circ f)_y = r_u u_y + r_v v_y$$ \footnote{Ableitungen nach u/v/x/y...}

\begin{definition}{Tangentialraum}
	Der \underline{Tangentialraum} $T_aF$ einer Fläche $F$ in einem Punkt $a \in F$ wird aufgespannt von $r_u(a)$ und $r_v(a)$
\end{definition}

\begin{center}
	\includegraphics[scale=0.5]{images/2012_01_19_Bild2.png}
\end{center}
eindimensional:
\begin{center}
	\includegraphics[scale=0.5]{images/2012_01_19_Bild3.png}
\end{center}

\begin{remark}
$T_a$ ist \underline{unabhängig} von der gewählten Parametrisierung!
\end{remark}

\begin{definition}{Einheitsnormalvektoren}
	Die Vektoren $\pm \frac{r_u \times r_v}{|r_u \times r_v|}$ heißen auch \underline{Einheitsnormalvektoren} von $F$ in $a$.
\end{definition}

\begin{remark}
	Diese stehen senkrecht auf $T_aF.$
\end{remark}

\begin{center}
	\includegraphics[scale=0.8]{images/2012_01_19_Bild4.png}
\end{center}
...
\begin{definition}{Bogenlänge}
	Die \underline{Bogenlänge} (oder \underline{Länge}) einer glatten Kurve in $F$ von $\gamma(a)$ zu $\gamma(b)$ ($t=a,b$) ist $$\int\limits_{a}^b{|\gamma^\prime(t)| dt} = \int\limits_a^b{\sqrt{\gamma^\prime \underbrace{\cdot}_{\text{Eukl. SKP im }\R^3} \gamma^\prime}dt}$$ 
	$$= \int\limits_a^b{\sqrt{(r_uu^\prime + r_v v^\prime) \cdot (r_u u^\prime + r_v v^\prime)}dt}$$
	$$= \int\limits_a^b{\sqrt{E {u^\prime}^2 + 2 F u^\prime v^\prime + G {v^\prime}^2} dt}$$
	mit $E := r_u \cdot r_u, F = r_u \cdot r_v, G = r_v \cdot r_v$
\end{definition}

\begin{definition}{erste Fundamentalform}
	Die \underline{erste Fundamentalform} einer Fläche im $\R^3$ ist der \underline{\underline{Ausdruck}}
	$$\RM{1} := E du^2 + 2 F du dv + G dv^2$$
	($= E du \otimes du + F du \otimes dv + G dv \otimes dv$)
\end{definition}

\begin{remark}
	$\RM{1}$ ist nichts anderes als die quadratische Form $Q(v,v):= v \cdot v$ \footnote{euklidisches Skalarprodukt} eingeschränkt auf den Tangentialraum von $F$, der von $r_u, r_v$ aufgespannt wird.
\end{remark}

\begin{example}
	Die Bogenlänge einer Kurve $\gamma(t) = (u(t), v(t))$ auf $F$ ist dann $\int\limits_a^b{\sqrt{E \left (\frac{du}{dt} \right)^2 + 2 F \frac{du}{dt} \frac{dv}{dt} + G \left ( \frac{dv}{dt} \right ) ^2} dt}$.
\end{example}

\paragraph{Motivation:} Ändert man die Parametrisierung von $F$ via $u(x,y), v(x,y)$ und wollten wir die Länge von $\gamma(t)$ (also dann $x(t), y(t)$) direkt bestimmen, müsste man berechnen
$$u^\prime = u_x x^\prime + u_y y^\prime, v^\prime = v_xx^\prime + v_y y^\prime \Rightarrow$$
$$E {u^\prime}^2 + 2 F u^\prime v^\prime + G {v^\prime}^2 = E (u_xx^\prime + u_y y^\prime)^2 + 2 F \ldots$$
$$= \ldots \text{\underline{\underline{furchtbar!}}}$$

Mit $du,dv$ schreibt man einfach 
$$du = u_x dx + u_y dy$$
$$dv = v_x dx + v_y dy$$
und substituiert man dies in $\RM{1}$, so erhält man einfach $E^\prime dx^2 + F^\prime dx dy + G^\prime dy^2$ mit $
\begin{pmatrix}
u_x & v_x \\ u_y & v_y
\end{pmatrix}
\begin{pmatrix}
...
\paragraph{} % jetzt die wirkliche VL
...
\begin{remark}
	Jeder Term in $(*)$ lässt sich auch durch $\RM{1}$ ausdrücken, denn: \newline
	z.B. gilt $\gamma_i^\prime = r_u u_i^\prime + r_v v_i^\prime$
	$$\Rightarrow \gamma_i^\prime \cdot \gamma_j^\prime = (r_u u_i^\prime + r_v v_i^\prime) (r_u u_j^\prime + r_v v_j^\prime)$$
	$$= E u_i^\prime v_j^\prime + F (v_i^\prime v_j^\prime + u_j^\prime v_i^\prime) + G v_i^\prime v_j^\prime$$
	\begin{center}
	\includegraphics[scale=0.4]{images/2012_01_24_Bild3.jpg}
\end{center}
\end{remark}

\begin{definition}{Flächeninhalt}
	Der \underline{Flächeninhalt} des parametrisierten Gebiets
	$r(V) \subset F \subset \R^3$ ist
	$$A = \int\limits_V{|r_u \times r_v| dudv} = \int\limits_V{\sqrt{EG - F^2} dudv}$$
\end{definition}

\begin{remark}
	$|r_u \times r_v|^2 = (r_u \cdot r_u)(r_v \cdot r_v) - (r_u \cdot r_v)^2 = EG - F^2$
\end{remark}
...
\section{Isometrien (von Flächen)}

\begin{definition}{isometrische Flächen}
	Zwei Flächen im $\R^3$ mit ersten Fundamentalformen heißen \underline{isometrisch}, wenn es einen Diffeomorphismus zwischen ihnen gibt, der Kurven auf Kurven gleicher Länge abbildet.
\begin{center}
	\includegraphics[scale=0.4]{images/2012_01_24_Bild5.jpg}
\end{center}
\end{definition}
...
\paragraph{Infos:}
\begin{itemize}
	\item Seminar über Differentialtopologie SoSe 2012 (ab 4. Semester), Anmeldung bei Frau Peters/ Dr. Grensing
	\item \underline{Vorbesprechung} Mo. 6.2. 18 Uhr A4-09 (Allianz)
	\item \underline{HEUTE} Anand Dessai: "\underline{Krümmung und Symmetrie}" Kolloquium Neuer Hörsaal (Architektur) 17:30 Uhr
\end{itemize}

\paragraph{Exkurs: Hyperbolische Geometrie}
\paragraph{Geometrische Motivation}
\begin{center}
	\includegraphics[scale=0.3]{images/2012_01_31_Bild1.jpg}
\end{center}
$F:=$Fläche($\Delta$) = $(\alpha + \beta + \gamma - \pi) \cdot R^2$
$$\Leftrightarrow \frac{\alpha + \beta + \gamma - \pi}{F(\Delta)} = \frac{1}{R^2}$$ \underline{\underline{unabhängig von $\Delta$ !}} \newline
$R \rightarrow \infty \leadsto$ Satz von Thales $\alpha + \beta + \gamma = \pi$ für Euklidische Dreiecke (in $\R^2$)

\begin{center}
	\includegraphics[scale=0.3]{images/2012_01_31_Bild2.jpg}
\end{center}
$$\lim\limits_{"\Delta \rightarrow \{p\}"}\frac{\alpha + \beta + \gamma - \pi}{F(\Delta)} =: K(p)$$ \underline{\underline{existiert}} und heißt \underline{\underline{Gauß-Krümmung}} der Fläche in $p$!

Verallgemeinerung der Gauß-Krümmung auf Mannigfaltigkeiten beliebiger Dimension:
\underline{\underline{Schnittkrümmung}}

\begin{definition}{Gauß-Krümmung}
Die \underline{Gauß-Krümmung} einer Fläche im $\R^3$ ist die Funktion
$$K := \frac{LN - M^2}{EG - F^2} = \frac{\det \RM{2}}{\det \RM{1}}$$
\end{definition}
...
\begin{theorem}[Theorema egregium; Gauß 1828]
\textcolor{red}{WARNUNG: Dieser Satz wurde geändert, weil die ursprüngliche Fassung falsch war!}

Die erste Fundamentalform bestimmt die Gaußkrümmung.
Insbesondere besitzen lokal isometrische Flächen dieselbe Gauß-Krümmung.
\footnote{Die ursprüngliche Fassung in der VL war: "Die zweite Fundamentalform ist durch die erste vollständig bestimmt. Insbesondere besitzen lokal isometrische Flächen dieselbe Gauß-Krümmung." Das stimmt so nicht, siehe Tut.}
\end{theorem}
...
\end{center}
	...
	\footnote{Durch Anwendung der Produktregel $(na)_u = na_u + n_ua$}
	bzw.
	$$\nabla_v a = a_v + (n_v \cdot a) n$$ 
	
	$\nabla_v$ und $\nabla_u$ hängen nur von $E,F,G$ ab!
	Es gilt
	$$\nabla_v \nabla_u = \nabla_v (\nabla_u a) = \nabla_v (a_u + (n_u \cdot a) n)$$
	$$= \nabla_v a_u + \nabla_v((n_u \cdot a) n) = a_{vu} - (n \cdot a_{vu}) n + \nabla_v((n_u a) n)$$
	$$= a_{vu} - (n \cdot a_{vu}) n + (n_u \cdot a) n_v,$$
	denn:
	\begin{itemize}
	...
	Vertauschen von $u$ und $v$ und $a_{uv} = a_{vn}$ liefert $$\nabla_v \nabla_u a - \nabla_u \nabla_v a = (n_u \cdot a) n_v - (n_v \cdot a) n_u = (n_u \times n_v) \times a.$$

	Es ist $n_u \times n_v = \lambda \cdot n$, und das bedeutet:
	$$\nabla_v \nabla_u - \nabla_u \nabla_v$$ "wirkt" auf $a$ wie folgt:
	$a$ wird in der Tangentialebene um $90 \deg$ gedreht und dann mit $\lambda$ multipliziert, wobei $\lambda$ intrinsisch (nur von $F/I$ abhängig) ist.
	
	$n_u \times n_v = \lambda \cdot n$ liefert ferner $\lambda n \cdot r_u \times r_v$
	$= (n_u \times n_v) \cdot (r_u \times r_v) = (n_u \cdot r_u) (n_v \cdot r_v) - (n_u \cdot r_v) (n_v \cdot r_u)$
	$= LN - M^2!$
	Außerdem ist $n \cdot r_u \times r_v = \sqrt{EG -F^2}$
	$$\Rightarrow \lambda = \frac{LN - M^2}{\sqrt{EG - F^2}} \Rightarrow LN - M^2 \text{ und somit auch $K$ hängen nur von $\RM{1}$ (und deren Ableitung) ab!}$$
\end{proof}
...
\begin{definition}{geodätische Krümmung}
	Die \underline{geodätische Krümmung} $\kappa_g$ einer glatten und nach Bogenlänge parametrisierten Kurve $\gamma$ in einer orientierten Fläche $M$ in $\R^3$ ist
	$$\boxed{\kappa_g := t^\prime \cdot n \times t}$$
	mit $t = \frac{d\gamma}{ds}, \gamma = \gamma(s)$
	\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_02_Bild1.jpg}
	\end{center}
\end{definition}

\begin{remark}
	$t = \gamma^\prime = \frac{d \gamma}{d s}$ ist tangential an $M$, der "Beschleunigungsvektor" $$t^\prime = \gamma^{\prime\prime} = \frac{d^2 \gamma}{d s^2}$$ im Allgemeinen nicht.
	\newline
	$\gamma$ ist nach Bogenlänge parametrisiert $\Rightarrow ||t|| = ||\gamma^\prime|| = 1$ \newline
	$\Rightarrow t \cdot t = 1 \Rightarrow t^\prime \cdot t = \frac{(t \cdot t)^\prime}{2} = 0$
	$\Rightarrow t^\prime \perp t \Rightarrow t^\prime = \alpha \underline{n} + \beta \underline{n \times t}$
	$\Rightarrow \kappa_g = \beta$ ist also die an $M$ tangentiale Komponente von $t^\prime$ und damit \underline{intrinsisch}! \footnote{d.h. eine nur von der Fläche selbst abhängige Größe}
	
	\paragraph{Konvention:} 
	Alle Kurven seien nach Bogenlänge parametrisiert.
\end{remark}

\begin{definition}{einfache geschlossene Kurve}
	Eine \underline{einfache geschlossene} Kurve in $M$ ist eine glatte Kurve in $M$, deren Bild zu $S^1$ homöomorph ist.
\end{definition}

\begin{theorem}[Gauß-Bonnet $\RM{1}$]
	Es sei $\gamma$ eine einfache geschlossene Kurve, die ganz in einer Kartenumgebung einer orientierten Fläche $M$ in $\R^3$ verlaufe und den Integrationsbereich $R$ umschließe. Dann gilt
	$$\int\limits_{\gamma = \partial R} \kappa_g ds = 2 \pi - \int\limits_{R}{K dA}$$
	\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_02_Bild2.jpg}
	\end{center}
	Dabei bezeichnet $dA$ das Flächenelement von $M$, $K$ die Gauß-Krümmung von $M$, $ds$ das Bogenlängeelement von $\gamma$ und $\kappa_g$ die geodätische Krümmung von $\gamma$.
\end{theorem}
...
\subsection{Der Satz von Gauß Bonnet (Globale Version)}

\paragraph{Erinnerung:}
\begin{itemize}
	\item $\gamma$ glatte und nach Bogenlänge parametrisierte Kurve in einer orientierten Fläche $M \subset \R^3$.
	\item \underline{Geodätische Krümmung} von $\gamma: \kappa_g := t^\prime \cdot n \times t, t = \frac{d\gamma}{ds}$
	\item \underline{Gauß-Bonnet $\RM{1}$}: $\gamma$ einfache geschlossene Kurve in einer Kartenumgebung einer orientierten Fläche $M \subset \R^3$, die den Bereich $R$ umschließt
	$\Rightarrow \int\limits_{\gamma = \partial R} \kappa_g ds = 2 \pi - \int\limits_{R} K dM, dM = dA= \text{Flächenelement}$ \footnote{$\gamma$ ist der Rand von $R$}
	\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_07_Bild1.jpg}
	\end{center}
	\item \underline{Verallgemeinerung:} $\gamma$ stückweise glatt 
	$\Rightarrow$ der Winkel $\Theta$ springt an jeder Ecke $i$ um den \underline{äußeren Winkel} $\delta_i$
	\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_07_Bild2.jpg}
	\end{center}
	$\Rightarrow \int \Theta^\prime(= 2 \pi \text{ in GB $\RM{1}$})$
	und ersetzt durch $\int\limits_\gamma \Theta^\prime ds = 2 \pi - \sum\limits_i \delta_i$
	$$= 2 \pi - \sum(\pi - \alpha_i) = (n \text{ Ecken }) = \sum \alpha_i - n \pi + 2 \pi$$
	$$= \sum\limits_i \alpha_i - (n-2) \pi \text{ mit den \underline{inneren Winkeln }} \alpha_i$$
\end{itemize}

\begin{corollary}
Die Winkelsumme eines (in einem Kartenbereich liegenden) "Dreiecks" (also eine stückweise glatten einfachen geschlossenen Kurve mit drei Ecken) auf einer orientierten Fläche im $\R^3$ ist $\alpha + \beta + \gamma = \pi + \int\limits_{R} K dM + \int\limits_\gamma \kappa_g ds$

\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_07_Bild3.jpg}
	\end{center}
\end{corollary}

\begin{example}
	\begin{enumerate}
		\item Gerade im $\R^2$ hat $\kappa_g = 0$, $\R^2$ hat $K=0$
		$\Rightarrow \alpha + \beta + \gamma = \pi$ (Thales)
		\item Ein Großkreis auf der Einheitssphäre $S^2$ erfüllt auch $\kappa_g = 0$:
		Ist beispielsweise $$\gamma(s) = (\cos s, \sin s, 0),$$ so gilt
		$$t = \frac{d \gamma}{d s} = (- \sin s, \cos s, 0) \Rightarrow t^\prime = (- \cos s, \sin s, 0)$$
		\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_07_Bild4.jpg}
	\end{center}
	$$\Rightarrow t^\prime \text{ steht senkrecht auf } S^2$$
	
	Wegen $K=1$ gilt für ein aus Großkreisstücken gebildetes Dreieck $\Delta$ auf $S^2$ damit $\alpha + \beta + \gamma = \pi + \int\limits_{\Delta} 1 d S^2 = \pi + \text{Fläche}(\Delta)$
	\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_07_Bild5.jpg}
	\end{center}
	\end{enumerate}
\end{example}

\begin{theorem}[Gauß-Bonnet für Flächen im $\R^3$]
	Für eine glatte und geschlossene orientierte Fläche $M \subset \R^3$ gilt
	$$\boxed{\int\limits_M{K \, \mathrm{d}M} = 2 \pi \cdot \chi(M)}$$
\end{theorem}

\begin{proof}
	Wähle eine glatte Triangulierung $T$ von $M$, so dass jedes Dreieck(sbild) $T_i$ mit stückweise glatten Seiten ganz in einer Kartenumgebung enthalten ist.
	\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_07_Bild6.jpg}
	\end{center}
	Es sei $n_0$ die Anzahl der Ecken, 
	
	$n_1$ die Anzahl der Seiten,
	
	$n_2$ die Anzahl der Flächen (Dreiecke)
	
	der Triangulierung $T$. Dann ist jede Seite die Kante von zwei Flächen, und jede Fläche hat drei Ecken, so dass gilt
	$$n_1 = \frac{3 n_2}{2}.$$
	\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_07_Bild7.jpg}
	\end{center}
	Für $\chi(M)$ gilt
	$$\chi(M) = \chi(T) = n_0 - n_1 + n_2$$
	$$= n_0 - \frac{3 n_2}{2} + n_2 = n_0 - \frac{n_2}{2}$$
	
	Für die Dreiecke $T_i$ auf $M$ gilt nun
	$$\int\limits_M K \, \mathrm{d} M = \sum\limits_{i=1}^{n_2}{\int\limits_{T_i} K \, \mathrm{d} M}.$$
	Aus der Formel für die Winkelsumme folgt bezüglich der Innenwinkel dann
	$$\sum\limits_i{\int\limits_{T_1}{K \, \mathrm{d} M}} = \sum\limits_{i=1}^{n_2}{(\alpha_i + \beta_i + \gamma_i - \pi)} - \sum\limits_{i=1}^{n_2}{\int\limits_{\partial T_i}{\kappa_g \, \mathrm{d} s}}$$
	Doch $\sum\limits_{i}^{n_2} \alpha_i + \beta_i + \gamma_i - \pi = \sum\limits_{i=1}^{n_2} \alpha_i + \beta_i + \gamma_i - n_2 \cdot \pi$
	
	$= 2 \pi n_0 - n_2 \cdot \pi$ durch Umordnen, denn die Innenwinkel
	\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_07_Bild8.jpg}
	\end{center}
	der Dreiecke addieren sich in jeder Ecke zu $2 \pi$.
	
	Andererseits gilt $\sum\limits_i{\int\limits_{\partial T_i} \kappa_g \, \mathrm{d} s = 0}$, denn die Terme auf gemeinsamen Seiten von Dreiecken haben sich gegenseitig auf!
	
	Also folgt $\chi(M) = n_0 - \frac{n_2}{2} = \sum\limits_{i=1}^{n_2} \int\limits_{T_i} K \, \mathrm{d} M - 0$
	$$= 2 \pi n_0 - n_2 \cdot \pi = 2 \pi \left(n_0 - \frac{n_2}{2}\right),$$
	d.h. $$\int K \, \mathrm{d} M = w \pi \cdot \chi(M)$$!
\end{proof}

\begin{remark}
Warum heißt $\kappa_g$ \underline{geodätische Krümmung}?

\underline{Geodätische} auf Flächen und Kurven, die Analoga zu Geraden in der Ebene sind.
\newline

Geraden lassen sich vorstellen als
\begin{enumerate}
	\item kürzeste Verbindungen (Abstände werden minimiert)
	\item "geradeste" Strecke (Tangentialvektor an Gerade ändert sich nicht)
	
		$\rightarrow t^\prime = 0$ (allgemeiner (im $\R^3$) $t^\prime \perp $ Fläche)
\end{enumerate}

\end{remark}

\begin{definition}{Geodätische}
	Eine \underline{Geodätische} auf einer Fläche $M \subset \R^3$ ist eine glatte Kurve $\gamma$ auf $M$, sodass in jedem Punkt $t^\prime$ normal \footnote{senkrecht} zur Fläche ist.
\end{definition}
...
\begin{example}
	\begin{enumerate}
		\item Ebene: Normale ist parallel zur Ebene, d.h.
		
		 $t^\prime$ ist normal $\Leftrightarrow t^\prime = 0 \Rightarrow \gamma = s \cdot a + b \Leftrightarrow \gamma$ ist gerade
		\item $ $
		\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_07_Bild9.jpg}
	\end{center}
	Großkreise auf $S^2$ sind Geodätische!
	
	\item Zylinder 
	\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_07_Bild10.jpg}
	\end{center}
	\end{enumerate}
\end{example}

\paragraph{Anwendungen von Gauß-Bonnet:}
\begin{itemize}
	\item $\chi(M)$ ändert sich nie $\Rightarrow \int K \, \mathrm{d} M \equiv c$
	\item Ist der Fundamentalsatz der Algebra falsch, so existiert eine $S^2 (= \C \Prim^1) \subset \R^3$ mit $K \equiv 0$ Widerspruch zu G.B.
\end{itemize}
\begin{center}
		\includegraphics[scale=0.3]{images/2012_02_07_Bild11.jpg}
	\end{center}
	
	\paragraph{(TODO: Exkurs: Topologie von schwarzen Löchern, foucaultsches Pendel, \ldots)}
\end{document}
